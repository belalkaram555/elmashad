...
const PlayStation: React.FC = () => {
  ...
  const devices = gamingDevices.filter(d => d.type === 'playstation');
  const sessions = gamingSessions.filter(s => s.deviceType === 'playstation');
  ...
  const [deviceForm, setDeviceForm] = useState({ name: '', hourlyRate: 20 });
  ...
  const syncDevice = async (op: 'create' | 'update' | 'delete', device: GamingDevice) => {
    ...
    if (isOnline()) {
      try {
        ...
        else { await idbDelete('gamingDevices', device.id); await api.gaming.removeDevice(device.id); }
  ...
  const handleOpenDeviceModal = (device?: GamingDevice) => {
    if (device) {
      ...
      setEditDevice(null);
      setDeviceForm({ name: '', hourlyRate: 20 });
    }
    setShowDeviceModal(true);
  };

  const handleSaveDevice = () => {
    if (!deviceForm.name.trim()) { addToast('يرجى إدخال اسم الجهاز', 'error'); return; }
    if (editDevice) {
      ...
      const newDevice: GamingDevice = { id: `ps_${Date.now()}`, name: deviceForm.name, type: 'playstation', hourlyRate: deviceForm.hourlyRate, status: 'available' };
  ...
  const handleDeleteDevice = (deviceId: string) => {
    if (gamingDevices.find(d => d.id === deviceId)?.status === 'in_session') {
      addToast('لا يمكن حذف جهاز في جلسة نشطة', 'error'); return;
    }
  ...
  const handleEndSession = () => {
    ...
    const order: Order = {
      ...
      items: [{
        ...
        nameAr: `جلسة بلايستيشن - ${endSession.deviceName}`,
        nameEn: `PlayStation session - ${endSession.deviceName}`,
        basePrice: amount,
        cost: 0,
        categoryId: 'gaming',
        image: '',
    ...
    const updatedDevice = gamingDevices.find(d => d.id === endSession.deviceId);
    if (updatedDevice) {
      const freeDevice: GamingDevice = { ...updatedDevice, status: 'available' };
      setGamingDevices(prev => prev.map(d => d.id === endSession.deviceId ? freeDevice : d));
      void syncDevice('update', freeDevice);
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-primary/10 rounded-2xl text-primary"><Gamepad2 size={24} /></div>
          <div>
            <h1 className="text-xl font-black text-textPrimary">جلسات البلايستيشن</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-surface p-4 rounded-2xl border border-cardAccent text-center">
          <p className="text-2xl font-black text-primary">{devices.filter(d => d.status === 'in_session').length}</p>
          <p className="text-xs text-secondary font-bold mt-1">أجهزة نشطة</p>
        </div>
        <div className="bg-surface p-4 rounded-2xl border border-cardAccent text-center">
          <p className="text-2xl font-black text-accentGreen">{devices.filter(d => d.status === 'available').length}</p>
          <p className="text-xs text-secondary font-bold mt-1">أجهزة متاحة</p>
        </div>
        <div className="bg-surface p-4 rounded-2xl border border-cardAccent text-center">
          <p className="text-2xl font-black text-accentBlue">{totalRevenue.toFixed(0)} {currency}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {devices.map(device => {
          ...
          return (
            <div key={device.id} className={`bg-surface rounded-[28px] border-2 p-5 transition-all ${isActive ? 'border-primary shadow-lg' : 'border-cardAccent'}`}>
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  <div className={`p-2 rounded-xl ${isActive ? 'bg-primary/15 text-primary' : 'bg-cardAccent text-secondary'}`}><Gamepad2 size={20} /></div>
                  <div>
              ...
              {isActive && activeSession ? (
                <div className="space-y-3">
                  <div className="bg-primary/5 rounded-2xl p-3 border border-primary/10 text-center">
                    ...
                    <p className="text-primary font-black text-xl">{currentAmount.toFixed(2)} {currency}</p>
                ...
                <button onClick={() => { setStartDevice(device); setShowStartModal(true); }} className="w-full flex items-center justify-center gap-2 py-3 bg-primary text-background rounded-2xl font-black text-sm hover:opacity-90 transition-all">
            ...
            <table className="w-full text-right">
              <thead>
                <tr className="text-secondary text-[10px] font-black uppercase border-b border-cardAccent">
                  <th className="pb-2 pr-2">الجهاز</th><th className="pb-2">العميل</th><th className="pb-2">المدة</th>
              ...
              <tbody>
                {completedSessions.slice(0, 20).map(s => (
                  <tr key={s.id} className="border-b border-cardAccent/30 hover:bg-background/40 transition-colors">
                    ...
                    <td className="py-2.5 font-black text-primary">{s.totalAmount?.toFixed(2)} {currency}</td>
      ...
      <Modal isOpen={showDeviceModal} onClose={() => setShowDeviceModal(false)}>
        <Modal.Header title={editDevice ? 'تعديل الجهاز' : 'إضافة جهاز جديد'} />
        <Modal.Body>
          <div className="space-y-4">
            <div>
              <label className="text-xs font-black text-secondary uppercase block mb-1.5">اسم الجهاز</label>
              <input autoFocus className="w-full p-3 bg-background border border-cardAccent rounded-xl text-textPrimary font-bold outline-none focus:border-primary"
                placeholder="مثلاً: PlayStation 4" value={deviceForm.name} onChange={e => setDeviceForm({ ...deviceForm, name: e.target.value })} />
            </div>
            <div>
        ...
        <Modal.Footer>
          <Button fullWidth onClick={handleSaveDevice}>{editDevice ? 'حفظ التعديلات' : 'إضافة الجهاز'}</Button>
        </Modal.Footer>
      </Modal>
          ...
          {endSession && (
            <div className="space-y-4">
              <div className="bg-primary/5 border border-primary/10 rounded-2xl p-5 text-center">
                ...
                <p className="text-4xl font-black text-primary">{calcAmount(getElapsedSeconds(endSession.startTime), endSession.hourlyRate).toFixed(2)} {currency}</p>
